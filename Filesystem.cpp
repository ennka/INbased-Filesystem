...
INode System::goalInode(string path, string &fileName, State &s)
{
	...
	if (temp == "") {
		s=NO_FILENAME;//此时的temp为文件名
		return current_Inode;
	}
		
	if ((int)temp.size() >= MAX_FILENAME_SIZE) {
		s = LENGTH_EXCEED;//此时的temp为文件名
		return current_Inode;
	}
	vect.push_back(temp);
...
State System::createFile(string fileName, int fileSize) {
	...
	bool ok = true;
	INode nextInode = findNextInode(inode, fName, ok);//fName是文件名
...
State System::deleteFile(string fileName) {
	...
	//已经到达目标目录，可以创建文件了
	deleteFileFromDentry(inode, fName);//在该目录所属的I节点下删除对应名字的文件
	inode = nextInode;
...
void System::deleteDirUsingInode(INode inode, string dName)
{
	...
	deleteEmptyDir(inode, dName);//最后一步，删除掉这个空文件夹的架构
}
...
State System::deleteDir(string dirName) {
	...
	//以上步骤已经确定抵达一个正确的 可被删除的文件夹

	deleteDirUsingInode(inode, vect[cnt - 1]);//删除掉整个文件夹。
